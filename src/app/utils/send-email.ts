...
export const SendEmail = async (
  ...
  일부 문자열이 일치하거나 포함된 경우일경우 해당 데이터를 배열에 넣어서 반환해 주세요. 
  해당 데이터를 배열에 넣은 근거를 설명할수없다면 데이터를 넣지마세요 또한
  ...
  try {
    ...
    if (emailData.length > 0) {
      ...
      일부 문자열이 일치하거나 포함된 경우일경우 해당 문의사항 데이터 객체를 배열에 넣어서 반환해 주세요. 
      또한 관련된 데이터가 없다면, 빈 배열만 반환해 주세요.
      비교 방식은 다음과 같습니다:
      text 값에 title이나 content의 일부 또는 전체가 포함되어 있는지 확인합니다.
      text 값하고 title이나 content의 값이 논리적으로 연관성이 있다고 생각되는 경우를 확인합니다.
      문자열 포함 여부를 확인할 때는 대소문자 구분 없이 비교해 주세요.
